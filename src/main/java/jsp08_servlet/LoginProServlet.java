...
public class LoginProServlet extends HttpServlet {    
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// => reqeust 객체의 getParameter() 메서드 활용
...
//			System.out.println("로그인 성공!");
			...
			// 세션 객체에 로그인 성공 아이디 저장(속성명 "sId")
			// 단, 자바에서는 내장 객체 session 이 존재하지 않으므로
			// request 객체로부터 session 객체를 얻어와서 사용해야 한다!
...
//			response.sendRedirect("/servlet1_main.jsp"); // http://localhost:8080/servlet1_main.jsp
			...
			// 컨텍스트 루트(StudyJsp)가 webapp 디렉토리를 가리키기 때문에
			// 컨텍스트 루트 뒤에 디렉토리 구조(/jsp08_servlet) 를 유지한 채 파일명만 지정하면 된다!
			// 따라서, 현재 위치에서 서블릿 주소만 교체하면 되므로 파일명만 지정 가능
			response.sendRedirect("servlet1_main.jsp");
			// => http://localhost:8080/StudyJsp/jsp08_servlet/servlet1_main.jsp 페이지로 디다이렉트
...
//			System.out.println("로그인 실패!");
			 ...
			 * => 응답 객체인 response 객체를 활용하여 응답 데이터 생성해야함
			 * 
			 * 1) 출력할 HTML 형식에 대한 문서 타입(contentType) 설정
			 *    => 응답 데이터 형식(타입)으로 HTML 태그가 사용됨을 클라이언트에게 알려줌
			 *    => response 객체의 setContentType() 메서드를 호출하여 문서 타입 지정
			 *       (JSP 파일 최상단 page 디렉티브의 contentType=XXX 항목 활용)
			 *       
			 * 2) 자바 코드를 사용하여 HTML 태그(자바스크립트 포함) 출력(전송)하려면
			 *    java.io.PrintWriter 객체 필요 (= 출력 스트림을 사용할 객체)
			...
			// 3) PrintWriter 객체(out)의 print() or println() 메서드로 출력할 HTML 태그 문자열 전달
			// => JSP의 out 객체와 동일하게 사용
			out.println("<script>");
