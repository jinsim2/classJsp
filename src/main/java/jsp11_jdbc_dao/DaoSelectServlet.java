...
//		System.out.println("이름 : " + student.getName());
		...
		//	  배열명.length 속성 대신 => List 객체의 size() 메서드 활용
//		for(int i = 0; i < studentList.size(); i++) {
//			// List 객체의 get() 메서드를 호출하여 저장된 StudentDTO 객체 1개 꺼내기
//			// => 파라미터 : 인덱스		리턴타입 : StudentDTO(student)
...
//		}
		...
		// --------------------------------------------------------------------------------------
		// 뷰페이지(select.jsp)로 포워딩 시 전체 레코드가 저장된 List 객체를 함께 전달해야하므로
		// request 객체의 setAttribute() 메서드를 호출하여 전송할 데이터를 저장해야 한다!
		// 이 때, request 객체가 다음 페이지까지 유지되어야 하므로 디스패치 방식 포워딩 필수!
		// => 속성명 "studentList" 로 List 객체 저장
		// => 이 때, setAttribute() 메서드의 파라미터 타입 Object 로 업캐스팅 일어남
		request.setAttribute("studentList", studentList);
//		request.setAttribute("student", student);
		
		// 학생 목록 정보를 표시하기 위해 jsp11_jdbc_dao/select.jsp 페이지로 포워딩(디스패치)
		...
		// 1. request 객체의 getReqeustDispatcher() 메서드를 호출하여 RequestDispatcher 객체 리턴받기
		//    => 파라미터 : 포워딩 될 이미지	리턴타입 : RequestDispatcher(dispatcher)
