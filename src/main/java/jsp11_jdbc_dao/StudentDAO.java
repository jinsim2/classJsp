...
//		System.out.println("StudentDAO - insert()");
		...
		try {
			...
			// DB연결(2단계) 까지의 처리는 JdbcUtil 클래스의 getConnection() 메서드에서
...
//		StudentDTO student = null;
		...
		try {
			...
			//    배열을 사용할 수 있으나 데이터 조회 결과가 몇 개의 레코드인지 모르므로
			//    배열을 생성할 수 없다! (자바의 배열은 크기 변경 불가능 = 불변)
...
//			List<StudentDTO> studentList = new ArrayList<StudentDTO>(); // 업캐스팅
...
//				System.out.println("번호 : " + idx + ", 이름 : " + name);
				...
				// 3) 복수개의 레코드(= 복수개의 StduentDTO 객체) 를 저장할 List 객체에
