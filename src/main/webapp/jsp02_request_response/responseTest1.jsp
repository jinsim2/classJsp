...
=> 따라서, 이 응답 데이터를 클라이언트가 수신하는 즉시 새로운 요청이 발생하므로
   ...
   리다이렉트 된ㄷ responseTest2.jsp 페이지의 내용을 최종적으로 볼 수 있게 된다!
*/
...
<%-- 단, HTML 태그(또는 자바스크립트 등)는 클라이언트에서 실행되며 --%>
<%-- 응답 데이터에 포함된 리다이렉트 신호에 의해 responseTest2.jsp 페이지로 요청이 수행되므로 --%>
<%-- 아래 모든 HTML 요소들은 클라이언트에서 확인(실행) 불가능하다! --%>
<script>
	console.log("확인"); // 코드 실행이 되지 않는다!
	alert("확인");
</script>