...
<body>
	...
	- 서버에 접속(연결) 시 기본적으로 해당 클라이언트에 해당하는 기억 장소(= 세션)가 생성되고
	  ...
	  => 주의! 로그인 할 때 사용하는 아이디가 아님(세션ID는 사용자가 결정하지 못함)
	- 세션 유지 또는 제거 조건
	...
	<hr>
	<%-- 세션 강제 초기화(=해당 클라이언트에 대한 세션 객체 제거) --%>
	<%session.invalidate(); %>
	<%-- 세션 초기화 후에는 세션 객체 접근이 불가능함 --%>
	<h3>새 세션 여부 : <%=session.isNew() %></h3> <%-- 이 코드 실행 시 오류 발생! --%>
	<%-- 새로운 세션이 생성되기 전까지는 접근 불가 --%>
	<%-- 다른 페이지 이동 등의 작업을 통해 서버와의 통신을 수행하면 새 세션 객체 생성됨 --%>
</body>
</html>